function scans_to_process = longDTI_load_rawdata( scandatafolder )
%longDTI_load_rawdata - Set up patient dataset to input into longitudinal processing
...
% To Do:
%
% Author: Suneth Attygalle & Gabe Marx
% Created 7/28/15
%
% Revisions:
...
allparticipants =longDTI_participant.empty(size(d,1),0); % initialize array of participants

for i = 1:size(d,1)
    allparticipants(i) = longDTI_participant(d(i).name, datapath); % fill array with participants data
end
