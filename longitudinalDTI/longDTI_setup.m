...
scandatafolder = '/mnt/macdata/groups/rosen/FTD_ddROI_long-proj/dti/data';

%path to dartel template for your study (or where a dartel template will
%be created if it doesn't exist yet):
templatepath = '/mnt/macdata/groups/rosen/FTD_ddROI_long-proj/dti/template';
...
scans_to_process = longDTI_load_rawdata( scandatafolder );


%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 2. Register timepoints to TPM
scans_to_process = longDTI_coregistertoTPM(scans_to_process); 

%% 3. Run longitudinal registration
scans_to_process=longDTI_run_long_registration(scans_to_process);

%% 4. DARTEL registration of Longitudinal Average Images to New DARTEL Template
scans_to_process = longDTI_DARTEL_registration_to_new(scans_to_process,templatepath);

%% 5. DARTEL registration of Longitudinal AverageImages to Existing DARTEL Template
scans_to_process = longDTI_DARTEL_create_warp_to_template(scans_to_process, templatepath);

%% 6. Generate population to ICBM registration deformation field
longDTI_generateDARTELToICBM(templatepath); % generates dartel pop to ICBM deformation field using SPM12

%%7. Warp timepoints to ICBM via AVG and DARTEL
scans_to_process=longDTI_warp_timepoint_to_ICBM_via_long( scans_to_process, templatepath );
